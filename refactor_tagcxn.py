...
class TagCXN:
    ...
    def tune_threshold(self, predictions, not_bce=False):
        ...
        #     steps = 1000
        #     print('Threshold bounds:', threshold_range)
        # data_all = utils.load_dict_data('full_dataset.csv', skip_head=True)
        for i in tqdm(np.arange(init_thr, 1, .01)):
            # threshold = init_thr + (float(i + 1) / steps)
            threshold = i
